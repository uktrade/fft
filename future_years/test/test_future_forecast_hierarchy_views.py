...
class ViewFutureForecastHierarchyWithOtherDataTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.test_user.user_permissions.add(can_view_forecasts)
        self.test_user.save()
        extra_year = get_current_financial_year() + 1
        # Create the figure for the year in the future
        self.forecast_data = MonthlyFigureSetup(extra_year)
        self.forecast_data.setup_forecast()
        self.forecast_data.setup_budget()
        self.test_year = extra_year + 1
        # Create the figure for the year in the future
    ...
    def test_view_group_summary(self):
        response = self.client.get(
            reverse(
                "forecast_group",
                kwargs={"group_code": self.forecast_data.group_code,
                        "period": self.test_year, },
            )
        )
