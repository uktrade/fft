...
class GiftsHospitalityCommandsTest(TestCase):
    ...
    def test_g_and_h_not_imported(self):
        self.assertFalse(
            GiftAndHospitalityCompany.objects.filter(sequence_no=10).exists()
        )
        self.assertFalse(
            GiftAndHospitalityCategory.objects.filter(sequence_no=10).exists()
        )
        self.assertFalse(
            GiftAndHospitalityClassification.objects.filter(sequence_no=10).exists()
        )
