...
SELECT 
       ...
       COALESCE(b.budget, 0)/100 as period_budget, 
	   COALESCE(f.actual, 0)/100 as period_actual,
	   COALESCE(f.forecast, 0)/100 as period_forecast,
	   ...
	   (COALESCE(f.actual, 0) + COALESCE(f.forecast, 0))/100 as period_actual_forecast,	
	   (COALESCE(b.budget, 0) - (COALESCE(f.actual, 0) + COALESCE(f.forecast, 0)))/100 as period_actual_forecast_budget_variance,
       COALESCE(b.ytd_budget, 0)/100 as ytd_budget,
 	   COALESCE(a_ytd.ytd_actual, 0)/100 as ytd_actual,
 	   COALESCE(fa_ytd.ytd_forecast_actual, 0)/100 as ytd_actual_forecast,
 	   (COALESCE(b.ytd_budget, 0) - COALESCE(fa_ytd.ytd_forecast_actual, 0))/100 as ytd_actual_forecast_budget_variance,
       COALESCE(p.previous_year_ytd, 0)/100 as previous_year_ytd,
	   COALESCE(b.budget_outturn, 0)/100 as full_year_budget,
 	   COALESCE(cy_o.current_year_outturn, 0)/100 as full_year_actual_forecast,
 	   (COALESCE(cy_o.current_year_outturn, 0) - COALESCE(b.budget_outturn, 0))/100 as full_year_actual_forecast_budget_variance,
 	   COALESCE(rates.run_rate_ytd, 0)/100 as ytd_run_rate,
  	   COALESCE(rates.full_year_run_rate, 0)/100 as full_year_run_rate,
	   COALESCE(fp.forecast, 0)/100 as previous_period_forecast,
 	   COALESCE(p.previous_year_outturn, 0)/100 as previous_year_outturn,
	   COALESCE(p.previous_year_actual, 0)/100 as previous_year_actual
