...
def generate_create_view_forecast_query(archived_period_name, archived_period_id):
    ...
    query = (
        f"Create view budget_forecast_{archived_period_name}_view as "
        f"SELECT ROW_NUMBER () "
        f"OVER (ORDER BY coalesce(b.financial_code_id, f.financial_code_id, o.financial_code_id)) as id,"
        f"coalesce(b.financial_code_id, f.financial_code_id, o.financial_code_id) as financial_code_id,"
        f"coalesce(b.financial_year_id, f.financial_year_id, o.financial_year_id) as financial_year, "
        f"coalesce(f.archived_period_id, b.archived_period_id, o.next_forecast_period_id) as archived_period_id, "
        ...
        f"coalesce(previous_outturn, 0) as previous_outturn "
        ...
        f"FULL OUTER JOIN end_of_month_monthlyoutturn o "
        f"on o.financial_code_id = f.financial_code_id "
        f"and o.financial_year_id = f.financial_year_id "
        f"and o.next_forecast_period_id = f.archived_period_id "
        f"WHERE coalesce(f.archived_period_id, b.archived_period_id,o.next_forecast_period_id) = {archived_period_id}; "
    )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunSQL(
            generate_create_view_forecast_query("apr", 1),
            generate_view_forecast_drop_query("apr"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("may", 2),
            generate_view_forecast_drop_query("may"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("jun", 3),
            generate_view_forecast_drop_query("jun"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("jul", 4),
            generate_view_forecast_drop_query("jul"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("aug", 5),
            generate_view_forecast_drop_query("aug"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("sep", 6),
            generate_view_forecast_drop_query("sep"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("oct", 7),
            generate_view_forecast_drop_query("oct"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("nov", 8),
            generate_view_forecast_drop_query("nov"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("dec", 9),
            generate_view_forecast_drop_query("dec"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("jan", 10),
            generate_view_forecast_drop_query("jan"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("feb", 11),
            generate_view_forecast_drop_query("feb"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("mar", 12),
            generate_view_forecast_drop_query("mar"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("adj1", 13),
            generate_view_forecast_drop_query("adj1"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("adj2", 14),
            generate_view_forecast_drop_query("adj2"),
        ),
        migrations.RunSQL(
            generate_create_view_forecast_query("adj3", 15),
