...
function EditForecast() {
  ...
  useEffect(() => {
    const capturePaste = (event) => {
      ...
      let crsfToken = document.getElementsByName("csrfmiddlewaretoken")[0]
        .value;
    ...
    const handleKeyDown = (event) => {
      ...
      if (event.key === "Enter") {
        if (document.activeElement.className === "link-button govuk-link") {
          ...
          "select_row_btn govuk-link link-button"
        ) {
      ...
      if (event.key === "Tab") {
        ...
        if (document.activeElement === filterOpenLink) {
          dispatch(OPEN_FILTER_IF_CLOSED());
          return;
        }
        ...
        if (targetRow > -1) {
          if (event.shiftKey) {
            ...
            if (!targetMonth) {
              // See if we're on a select button
              if (targetRow === 0) {
                // See if we're at the start of the table
                dispatch(
                  SET_EDITING_CELL({
                    cellId: null,
                  }),
              ...
              // We're coming from a cell
              if (targetMonth === lowestMonth + 1) {
                // See if we need to jump to select link
                let selectRowBtn = document.getElementById(
                  "select_row_" + targetRow,
                );
                selectRowBtn.focus();
                event.preventDefault();
                dispatch(
            ...
            // Going forwards
            if (!targetMonth) targetMonth = lowestMonth;
        ...
        >
          ...
          <div className="govuk-error-summary__body">
            <ul className="govuk-list govuk-error-summary__list">
              <li id="paste_error_msg">{errorMessage}</li>
            </ul>
          </div>
        </div>
      )}
