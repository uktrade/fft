...
const CostCentrePayrollList = ({rowIndex, cellKey, format}) => {
        ...
        <Fragment>
            ...
            <ul className="cost-centre-list">
              {displayedCentres.map((costCentre, index) => {
                return <li key={index}>
                    <a href={ `/payroll/edit/${costCentre.code}/${forecastFinYear}` } className="govuk-link">{costCentre.code} - {costCentre.name}</a>
                </li>
              })}
            </ul>
        </Fragment>
...
export default CostCentrePayrollList
