...
class ImportNACFieldsTest(BaseTestCase):
    ...
    def test_import_nac_fields(self):
        natural_account_obj = NaturalCode.objects.get(natural_account_code=self.natural_account_code_1)
        self.assertIsNone(natural_account_obj.gross_income)
        assert(natural_account_obj.cash_non_cash==NaturalCode.NOT_DEFINED)

        natural_account_obj = NaturalCode.objects.get(natural_account_code=self.natural_account_code_2)
        self.assertIsNone(natural_account_obj.gross_income)
        assert(natural_account_obj.cash_non_cash==NaturalCode.NOT_DEFINED)
