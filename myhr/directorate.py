...
class DirectorateView(TemplateView):
    ...
    def get_all_directorate_fte_sum_and_count(self, group_name=None):
        # Get all directorate FTE sum and count based on group name
        all_directorates = HRModel.objects.filter(group=group_name).values('directorate').annotate(
            total_fte=Sum('fte'), total_count=Count('fte'))

        fte_sum_and_count = []
        for group in all_directorates:
            fte_sum_and_count.append({
                'group': group['directorate'],
    ...
    def get_groups_serialiser(self, group_name=None):
        get_all_groups_data = self.get_all_directorate_fte_sum_and_count(group_name)
    ...
    def get_context_data(self, **kwargs):
        ...
        logger.info(f"directorate data: {group_data}")

        context = super().get_context_data(**kwargs)
        context['group'] = 'MyHR'
        context['myhr_group_data'] = group_data
        context['myhr_next_view'] = 'costcentre'
        return context